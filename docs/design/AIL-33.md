...
| 项 | 说明 |
|---|---|
| `design/index.html` 入口导览 | **不作为产品路由**；仅保留在 `design/` 作原型索引。已登录默认进 `/dashboard`，未登录进 `/auth` |
...
邀请 → admin 创建 invited User →（演示）Toast；接受邀请首期可简化为「直接设密码激活」或仅保持 invited 状态
...
| 风险 / 待决 | 建议默认 | 需评审确认？ |
|---|---|---|
| 邀请「接受」完整链路（邮件落地页） | 首期只建 `invited` 记录，不做接受页 | 是 |
| Session 表 vs 纯无状态 JWT | **有 Session 表**，支持撤销与改密失效 | 否（已选） |
| 仪表盘数据真伪 | seed/mock API | 否 |
| 导出 CSV | P1，可用 Toast | 否 |
| MFA | UI 开关入库，无 TOTP | 否 |
| `index` 导览是否做成 `/` 营销页 | **不做**，`/` 重定向 | 是（若品牌需要可改） |

### 待评审聚焦问题（最多 2 个）

1. **邀请接受**：首期是否接受「仅创建 invited、无邮件/无接受页」？还是必须做最小接受落地（设密激活）？
2. **`/` 路由**：保持「登录态分流重定向」即可，还是需要单独落地页（类似 `index.html` 导览）？
...
- [ ] 人类评审通过并留下明确结论评论
- [ ] 评审通过后评论中 @全栈工程师，方可进入 Stage 2+ 编码
